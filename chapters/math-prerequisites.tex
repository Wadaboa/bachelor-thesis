...
\begin{equation}
	\label{eq:erosion}
	[\epsilon_{Q_{SE}}(Q_{I})](\textbf{x}) = \underset{\textbf{s}\in Q_{SE}}{\bigwedge}(Q_{I}(\textbf{x}+\textbf{s}))
\end{equation}
...
\begin{equation}
	\label{eq:dilation}
	[\delta_{Q_{SE}}(Q_{I})](\textbf{x}) = \underset{\textbf{s}\in Q_{SE}}{\bigvee}(Q_{I}(\textbf{x}+\textbf{s}))
\end{equation}
...
\begin{equation}
	...
	[\epsilon_{Q_{SE}}(Q_{F})]^{(i)}_{Q_{I}}(\textbf{x}) = [\epsilon_{Q_{SE}}([\epsilon_{Q_{SE}}(Q_{F})]^{(i-1)}_{Q_{I}}(\textbf{x}))]^{(0)}_{Q_{I}}(\textbf{x}), i=1,2,3,\dots,
\end{equation}
mentre la \textit{dilatazione geodetica}:
\begin{equation}
	\label{eq:geodesic-dilation}
	[\delta_{Q_{SE}}(Q_{F})]^{(i)}_{Q_{I}}(\textbf{x}) = [\delta_{Q_{SE}}([\delta_{Q_{SE}}(Q_{F})]^{(i-1)}_{Q_{I}}(\textbf{x}))]^{(0)}_{Q_{I}}(\textbf{x}), i=1,2,3,\dots
\end{equation}
Concettualmente, le iterazioni sopra definite potrebbero continuare indefinitamente, ma per ragioni pratiche si sceglie di farle terminare per un intero \textit{n} tale che nessun cambiamento avvenga $\forall n' > n$. L'output stabile $R^{\epsilon}_{Q_{I}}(Q_{F}))=[\epsilon_{Q_{SE}}(Q_{F})]^{(n)}_{Q_{I}}(\textbf{x})$ e $R^{\delta}_{Q_{I}}(Q_{F}))=[\delta_{Q_{SE}}(Q_{F})]^{(n)}_{Q_{I}}(\textbf{x})$ cos\`i definito viene denominato rispettivamente \textit{erosione} e \textit{dilatazione} con \textit{ricostruzione}.
...
\begin{equation}
	...
	\tilde{\gamma}_{R}(Q_{I}) = \gamma_{R}^{(n)}(Q_{I}) = R_{Q{I}}^{\delta}[\epsilon^{(n)}(Q_{I})]
\end{equation}
\begin{equation}
	\label{eq:closing-reconstruction}
	\tilde{\phi}_{R}(Q_{I}) = \phi_{R}^{(n)}(Q_{I}) = R_{Q{I}}^{\epsilon}[\delta^{(n)}(Q_{I})]
\end{equation}
