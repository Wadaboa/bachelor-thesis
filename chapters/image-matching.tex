...
\end{table}
Come si pu\`o notare dai risultati in tabella \ref{tab:feature-matching-comparison}, \textit{SURF} risulta essere il miglior \textit{feature matcher} in termini di accuratezza, mentre \textit{BRISK} \`e quello che performa meglio dal punto di vista temporale. Come gi\`a anticipato, in casi d'uso reali \textit{AKAZE} restituisce il miglior rapporto qualit\`a/tempo di esecuzione.\par
